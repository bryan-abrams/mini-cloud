...
| **vault** | Secrets; Consul ACL token issuance and Nomad ACL token issuance (both configured during setup). | **Consul** (for ACL integration). No other containers depend on Vault for startup. |
