...
docker exec -i gitea-postgres psql -U gitea -d postgres < "$SCRIPT_DIR/ensure-concourse-user.sql"
echo "Ensuring concourse database exists..."
docker exec gitea-postgres psql -U gitea -d postgres -c "CREATE DATABASE concourse OWNER concourse" 2>/dev/null || true
